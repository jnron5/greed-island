...
Zone layout lives in the scene files, plus `scripts/world/world_map.gd` (origins, towns, edges). Lake Veyra is generated by `python scripts/tools/build_lake_veyra.py` (edit the layout there, not in the editor). Gated pickups set `behind_gate` so rivals only take cards they can reach or pay for.
