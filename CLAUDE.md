...
Zone layout lives in the scene files, plus `scripts/world/world_map.gd` (origins, towns, edges). Lake Veyra is generated by `python scripts/tools/build_lake_veyra.py` (edit the layout there, not in the editor). Thornveil-biome ground (Thornveil, Sorenda, Warden's Grove) is painted from the PixelLab forest-path Wang tileset by `python scripts/tools/build_ground.py` — paths/plazas are shapes in that script, it colour-grades to the Thornveil palette and scatters `ForestTree*` nodes; re-run after changing a layout. Gated pickups set `behind_gate` so rivals only take cards they can reach or pay for.
