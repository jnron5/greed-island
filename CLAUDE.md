...
**Environment style (current direction): modern multilevel towns — Sea of Stars (Sabotage Studio) is the reference.** Towns are built on terraces and cliffs at several *real* walkable heights joined by stairs/ramps (not one flat plane with scattered props), with dense hand-crafted detail, buildings built into the terrain, and lighting/atmosphere (lamp and window glow, water shimmer, time-of-day tint). Reference is for style only — no names/assets from it.
...
Zone layout lives in the scene files, plus `scripts/world/world_map.gd` (origins, towns, edges). Lake Veyra is generated by `python scripts/tools/build_lake_veyra.py` (edit the layout there, not in the editor). Thornveil-biome ground (Thornveil, Sorenda, Warden's Grove) is painted from the PixelLab forest-path Wang tileset by `python scripts/tools/build_ground.py` — paths/plazas are shapes in that script, it colour-grades to the Thornveil palette and scatters `ForestTree*` nodes; re-run after changing a layout. Kalmora is a **multilevel** town generated entirely by `python scripts/tools/build_kalmora.py`: a corner heightmap (sea / quay / market terrace / upper town) is composed from three chained PixelLab **pro cliff tilesets** by `scripts/tools/terrain.py` (3-row cliff walls, stairs cut through them), plus collision, a level map, districts, and the 59-item prop pack. Edit the layout in that script, not the editor. **Elevation rules:** `Zone.level_at()` reads the level map; hits, sight, stealth steals and Pickpocket only work between characters on the same level. `Zone.find_path()` (AStarGrid2D over the zone's World collision) gives rivals real routes up stairs and around obstacles. New static props: `python scripts/tools/make_prop.py <name> <texture> <footprint w> <h>`. Zones clamp the player camera to their ground sprite (`GroundTiles`/`Ground`). Gated pickups set `behind_gate` so rivals only take cards they can reach or pay for.
